...
```
ユーザー入力 (シンボル + トレードスタイル)
  → /api/analyze (メインオーケストレータ)
  → スタイルに応じたタイムフレーム決定 (trading-style.ts)
...
| 群集心理 | `crowd-psychology.ts` | スクイーズリスク、FOMO/パニック検出、逆張りシグナル |
| トレードスタイル | `trading-style.ts` | スキャルピング/デイトレード/スイングのプリセット定義 |
...
- **SL**: 構造ベース (直近S/Rレベル) + ATRガードレール (0.3×〜1.5×ATR)。S/Rが見つからない場合は0.7×ATRをフォールバックに使用。OI残存率が高い場合はSL幅を1.2倍に拡大。
- **トレードスタイル**: スキャルピング / デイトレード / スイング の3プリセット
  - スキャルピング: 5m/15m/1h (環境認識:1h, セットアップ:15m, トリガー:5m)
  - デイトレード: 5m/15m/1h/4h (環境認識:4h, セットアップ:1h, トリガー:15m)
  - スイング: 15m/1h/4h/1d (環境認識:1d, セットアップ:4h, トリガー:1h) ※デフォルト
- **タイムフレーム重み付け**: トレードスタイルに応じて動的に変更
- **信頼度スコア**: 0-100 (weak/moderate/strong) + 不足データの明示
- **階層分析**: 環境認識足バイアス → セットアップ波動 → トリガー戦略 → 執行タイミング（ロールベース）
- **階層フィルター**: 環境認識足バイアスに逆行するエントリーをwaitに降格
- **心理的節目**: S/Rレベルに千ドル単位($1K/$5K/$10K)の丸い数字ボーナス
- **OI残存率**: 価格変動に対するOI残存率から再急変動リスクを予測
- **群集心理**: スクイーズリスク/FOMO/パニックの検出と逆張り調整
- **実験的調整**: ニュース感情による結論修正
